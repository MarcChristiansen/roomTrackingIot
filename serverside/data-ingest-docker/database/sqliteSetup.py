...
def setupDB(name):
    ...
    cur.execute('''CREATE TABLE occupancy (timestamp Integer, location TEXT, occupied INTEGER)''')
    cur.execute('''CREATE TABLE devices  (timestamp Integer, location TEXT, count Integer)''')
    cur.execute('''CREATE TABLE motion (timestamp Integer, device TEXT, location TEXT, motion Integer)''')
